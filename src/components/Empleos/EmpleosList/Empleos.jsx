...
function Empleos({ job }) {
  ...
  return (
    <article onClick={() => onSetJobID(job.id)} className="job flex flex-col md:flex-row justify-around lg:justify-center items-start p-4 h-52 md:h-48 w-full lg:w-[550px] cursor-pointer">
      <figure className="job__logo flex justify-around md:block md:h-44 w-full md:w-1/6 lg:w-32 md:pt-12 gap-2">
        <img src={job.company.logo} alt={job.company.name} className="w-1/4 md:w-full object-contain" />
